...
app.put('/api/cms/case-studies/:id', protect, async (req, res) => {
  try {
    ...
    const rows = await sql`UPDATE cms_case_studies SET title=${title}, client=${client}, industry=${industry}, service=${service}, excerpt=${excerpt}, challenge=${challenge}, solution=${solution}, result=${result}, metrics=${metricsJson}, published=${published}, featured_image=${featured_image}, "updatedAt"=NOW() WHERE id=${req.params.id} RETURNING *`;
    res.json({ success: true, caseStudy: rows[0] });
  } catch (e: any) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.post('/api/cms/case-studies', protect, async (req, res) => {
  try {
    ...
    const rows = await sql`INSERT INTO cms_case_studies (title, client, industry, service, excerpt, challenge, solution, result, metrics, published, featured_image) VALUES (${title}, ${client}, ${industry}, ${service}, ${excerpt}, ${challenge}, ${solution}, ${result}, ${metricsJson}, ${published}, ${featured_image}) RETURNING *`;
    res.json({ success: true, caseStudy: rows[0] });
  } catch (e: any) { res.status(500).json({ success: false, message: e.message }); }
});
